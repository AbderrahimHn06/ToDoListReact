    ...
    return (
      <Box sx={{ display: 'flex', flexDirection: 'row', gap: 1 }}>
        <Fab
          ...
          sx={fabStyle}
          onClick={() =>
            data.status == 'current' ? handleDone(data.id) : handleUndo(data.id)
          }
        >
          {data.status == 'current' ? (
            <CheckIcon sx={{ color: '#abb25eff' }} />
          ) : (
            <UndoIcon />
          )}
        </Fab>
  ...
  return (
    <div style={styles.card}>
      <Accordion style={styles.details}>
        ...
        >
          <Typography component="span" style={styles.todotitle}>
            {data.title}
          </Typography>
        </AccordionSummary>

        <AccordionDetails style={styles.todoinfo}>
          {data.description}
        </AccordionDetails>
      </Accordion>
